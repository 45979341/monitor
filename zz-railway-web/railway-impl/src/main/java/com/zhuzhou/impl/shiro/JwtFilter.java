...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean shouldTokenRefresh(Date issueAt){
        ...
        return LocalDateTime.now().minusSeconds(JwtUtil.REFRESH_TIME).isAfter(issueTime);
    }
    ...
    protected boolean onLoginSuccess(AuthenticationToken token, Subject subject, ServletRequest request, ServletResponse response) {
        ...
        if(token instanceof JwtToken){
            ...
            boolean shouldRefresh = shouldTokenRefresh(JwtUtil.getExpire(jwtToken.getToken()));
            if(shouldRefresh) {
                newToken = JwtUtil.sign(user.getLoginName(), user.getSalt());
                httpResponse.setHeader(ConstUtils.AUTH_HEADER, newToken);
            }
        }
